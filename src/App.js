...
import MatchGameTabsList from './components/MatchGameTabsList'
import MatchGamePhotos from './components/MatchGamePhotos'
import './App.css'
...
class App extends Component {
  ...
  setActivePhotoId = id => {
    ...
    if (activeImageUrl.id === id) {
      this.setState(prevState => ({
        activeImageUrl: randomNumber,
  ...
  render() {
    const {activeTabId, activeImageUrl, score, timer, isActivated} = this.state

    const filteredApps = this.getActiveTabApps()
    return (
      <div className="bg-container">
        <ul>
          <MatchGameNavbar score={score} timer={timer} />
        </ul>
        {isActivated ? (
          <>
            <div className="match-image-container">
              <img
                src={activeImageUrl.imageUrl}
                alt="match"
            ...
            <ul className="apps-list">
              <div className="image-container">
                {filteredApps.map(eachApp => (
                  <MatchGamePhotos
                    key={eachApp.thumbnailUrl}
                    appDetails={eachApp}
                    setActivePhotoId={this.setActivePhotoId}
                  />
                ))}
              </div>
            </ul>
